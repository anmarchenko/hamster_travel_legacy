  ...
  describe '#update' do
    ...
    context 'caterings' do
      ...
      it 'creates catering with number of days equal to trip length' do
        ...
        expect(trip.caterings.first.days_count).to eq(8)
      end

      it 'updates catering days count when days are changed' do
        Updaters::Trip.new(trip).update(start_date: Date.today, end_date: Date.today + 4.days)
        expect(trip.caterings.count).to eq(1)
        expect(trip.caterings.first.days_count).to eq(5)
      end

      it 'does not update catering days count when trip is empty' do
        Updaters::Trip.new(empty_trip).update(start_date: Date.today, end_date: Date.today + 4.days)
        expect(empty_trip.caterings.count).to eq(0)
      end

      it 'does not update catering days count when there are several caterings' do
        Updaters::Trip.new(trip_with_caterings).update(start_date: Date.today, end_date: Date.today + 4.days)
      ...
      it 'does not update catering days count when trip dates were not changed' do
        Updaters::Trip.new(trip_with_sepcific_catering).update(start_date: trip_with_sepcific_catering.start_date,
