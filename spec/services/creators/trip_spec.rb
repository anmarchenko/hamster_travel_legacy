...
RSpec.describe Creators::Trip do
  describe '#new_trip' do
    subject { Creators::Trip.new(original_trip).new_trip }

    context 'no original trip to copy from' do
      let(:original_trip) { nil }
  ...
  describe '#create_trip' do
    ...
    subject { Creators::Trip.new(original_trip).create_trip(params, user) }
    ...
    context 'original trip' do
      let(:original_trip) { FactoryGirl.create(:trip, :with_filled_days) }
