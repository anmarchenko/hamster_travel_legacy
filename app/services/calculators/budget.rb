  ...
  class Budget
    ...
    def transfers_hotel_cents
      Rails.cache.fetch(cache_key('transfers_hotel'), expires_in: 1.day.to_i) do
        ...
        (trip.days.includes(
          :hotel, :transfers, :activities, :expenses
        ) || []).each do |day|
          result += day.hotel.amount.exchange_to(
            CurrencyHelper::DEFAULT_CURRENCY
          )
          (day.transfers || []).each do |transfer|
            result += transfer.amount.exchange_to(
              CurrencyHelper::DEFAULT_CURRENCY
            )
          end
        end
    ...
    def activities_other_cents
      Rails.cache.fetch(
        cache_key('activities_other'), expires_in: 1.day.to_i
      ) do
        result = Money.new(0, CurrencyHelper::DEFAULT_CURRENCY)
        (trip.days.includes(
          :hotel, :transfers, :activities, :expenses
          ...
          (day.activities || []).each do |activity|
            result += activity.amount.exchange_to(
              CurrencyHelper::DEFAULT_CURRENCY
            )
          end
          (day.expenses || []).each do |expense|
            result += expense.amount.exchange_to(
              CurrencyHelper::DEFAULT_CURRENCY
            )
          end
        end
    ...
    def exchange(integer_result)
      Money.new(
        integer_result, CurrencyHelper::DEFAULT_CURRENCY
      ).exchange_to(currency).to_f
    end
